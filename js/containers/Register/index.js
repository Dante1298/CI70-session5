...
import { checkEmail, checkPassword, checkUserName } from "../../common/validation.js";
...
class RegisterScreen {
  ...
  constructor() {
    ...
    this.$link.innerText = "I already have an account!";
    this.$link.classList.add("d-block", "link");
    this.$link.addEventListener("click", this.handleChangeSreen);
  ...
  handleSubmit = async(e) => {
    e.preventDefault();
    const { email, password, rePassword, uName } = e.target;
    let isError = false;
    if (checkUserName(uName.value) !== null) {
        this.$userName.setError(checkUserName(uName.value));
        isError = true;
    } else {
    ...
    if (checkPassword(rePassword.value) !== null) {
        ...
        isError = true;
         ...
         this.$rePassword.setError("Your re-password is not match.");
        isError = true;
      }
       else { 
